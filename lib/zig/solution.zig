...
pub const Solution = struct {
    pub fn solve(self: *const Solution, allocator: std.mem.Allocator, data: types.ProblemInput, version: u8, is_sample: bool) !types.TimedResult {
        const start_time = std.time.nanoTimestamp();
        const result = try switch (version) {
            1 => self.solve_version01(allocator, data, is_sample),
            2 => self.solve_version02(allocator, data, is_sample),
            // 2 => try day2.solve(),
            // Add more days as needed
            else => types.SolvingError.InvalidVersion,
        };
    ...
    solve_version01: *const fn (std.mem.Allocator, types.ProblemInput, bool) types.Result,
    solve_version02: *const fn (std.mem.Allocator, types.ProblemInput, bool) types.Result,
};
